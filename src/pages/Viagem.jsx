      ...
      <header className="bg-[#112240] border-b border-[#1E3A5F] px-5 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-600 text-[11px]">{profile?.nome}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {!isOnline && <WifiOff size={16} className="text-yellow-400" />}
          <button onClick={async () => { await signOut() }} className="text-slate-500 hover:text-red-400 transition-colors p-1">
            <LogOut size={18} />
          </button>
        </div>
      </header>
            ...
            <div key={n} className="flex items-center">
              <div className="flex flex-col items-center gap-1">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                  done ? 'bg-orange-500 text-white' : active ? 'bg-orange-500/20 text-orange-400 ring-2 ring-orange-500' : 'bg-[#112240] text-slate-600'
                }`}>
                  {done ? <CheckCircle size={14} /> : n}
                </div>
                <span className={`text-[10px] font-medium ${active ? 'text-orange-400' : done ? 'text-slate-400' : 'text-slate-600'}`}>{label}</span>
              </div>
              {i < 3 && <div className={`w-7 h-px mb-5 mx-1 ${n < step ? 'bg-orange-500' : 'bg-[#1E3A5F]'}`} />}
            </div>
          )
        })}
      </div>
          ...
          return (
            <button key={item.id} onClick={() => onSelecionar(item)}
              className={`w-full flex items-center justify-between px-4 py-4 rounded-2xl border transition-all ${
                selected ? 'bg-orange-500/10 border-orange-500' : 'bg-[#112240] border-[#1E3A5F] hover:border-orange-500/40'
              }`}>
              <div className="flex items-center gap-3">
                <Truck size={18} className={selected ? 'text-orange-400' : 'text-slate-600'} />
                <div className="text-left">
                  <p className={`font-mono font-semibold text-sm ${selected ? 'text-white' : 'text-slate-300'}`}>{item.placa}</p>
                  <p className="text-slate-500 text-xs mt-0.5">{item.tipo}</p>
                </div>
              </div>
              {selected && <CheckCircle size={18} className="text-orange-400" />}
            </button>
          )
        })}
        {!itens.length && <p className="text-slate-600 text-sm text-center py-8">Nenhum cadastrado no sistema</p>}
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] overflow-hidden">
        <div className="flex gap-2 p-3">
          <input value={searchNum} onChange={e => setSearchNum(e.target.value)}
            ...
            className="flex-1 bg-[#0B1929] border border-[#1E3A5F] rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-orange-500" />
          <button onClick={buscarPedido} disabled={searching || !searchNum.trim()}
            className="bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white px-4 rounded-xl font-semibold text-sm transition-colors flex items-center gap-2">
            {searching
        ...
        {searchResult?.ok && (
          <div className="mx-3 mb-3 bg-[#0B1929] rounded-xl p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-orange-400 font-mono text-sm font-semibold">#{searchResult.ok.numero_pedido}</p>
                <p className="text-slate-400 text-xs mt-0.5">{searchResult.ok.unidade?.codigo} · {searchResult.ok.fabrica}</p>
                <p className="text-slate-600 text-xs">{searchResult.ok.itens.length} produtos · {searchResult.ok.total_pallets.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} pallets</p>
              </div>
              <button onClick={adicionarPedido}
                className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold px-3 py-1.5 rounded-xl flex items-center gap-1 transition-colors shrink-0">
      ...
      {pedidosAdicionados.length > 0 && (
        <div className="space-y-2">
          {pedidosAdicionados.map(p => (
            <div key={p.numero_pedido} className="bg-[#112240] rounded-2xl border border-[#1E3A5F] px-4 py-3 flex items-center justify-between gap-3">
              <div>
                <p className="text-orange-400 font-mono text-sm font-semibold">#{p.numero_pedido}</p>
              ...
              <button onClick={() => removerPedido(p.numero_pedido)} className="text-slate-600 hover:text-red-400 transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
          ))}
          <p className="text-right text-xs text-slate-600">
            Total: <span className="text-white font-semibold">{totalPallets.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}</span> pallets · <span className="text-white font-semibold">{totalSkus.toLocaleString('pt-BR')}</span> caixas
          </p>
        </div>
      )}
      ...
      {unidadesNaViagem.length > 1 && (
        <div className="bg-[#112240] rounded-2xl border border-orange-500/40 p-4 space-y-3">
          <p className="text-orange-400 text-sm font-semibold">Para qual unidade vai descarregar?</p>
          {unidadesNaViagem.map(u => (
            <button key={u.id} onClick={() => setUnidadeDescarga(u)}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${
                unidadeDescarga?.id === u.id ? 'bg-orange-500/10 border-orange-500 text-white' : 'bg-[#0B1929] border-[#1E3A5F] text-slate-400'
              }`}>
              <div className="flex items-center gap-2">
                <MapPin size={14} className={unidadeDescarga?.id === u.id ? 'text-orange-400' : 'text-slate-600'} />
                <span className="text-sm font-medium">{u.nome} — {u.cidade}</span>
              </div>
              {unidadeDescarga?.id === u.id && <CheckCircle size={15} className="text-orange-400" />}
            </button>
          ))}
        </div>
      )}
      ...
      {unidadesNaViagem.length === 1 && unidadeDescarga && (
        <div className="flex items-center gap-2 bg-[#112240] rounded-xl border border-[#1E3A5F] px-4 py-2.5">
          <MapPin size={14} className="text-orange-400 shrink-0" />
          <p className="text-slate-400 text-xs">Destino: <span className="text-white font-semibold">{unidadeDescarga.nome} — {unidadeDescarga.cidade}</span></p>
        </div>
      )}
      ...
      <div>
        ...
        <input type="time" value={horarioAgendado} onChange={e => setHorario(e.target.value)}
          className="w-full bg-[#112240] border border-[#1E3A5F] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-orange-500 [color-scheme:dark]" />
      </div>
...
function StepConfirmar({ carreta, cavalo, pedidosAdicionados, unidadeDescarga, horarioAgendado, onVoltar, onConfirmar, iniciando }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-white font-semibold text-base">Confirmar Viagem</h2>

      <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] divide-y divide-[#1E3A5F]">
      ...
      <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] overflow-hidden">
        ...
        {allItens.slice(0, 5).map(item => (
          <div key={item.id} className="flex items-center justify-between px-4 py-2.5 border-b border-[#0B1929] last:border-0">
            <p className="text-slate-300 text-xs truncate flex-1 mr-4">{item.descricao}</p>
            <p className="text-xs text-slate-500 shrink-0">{Number(item.qtde_pallets).toLocaleString('pt-BR', { maximumFractionDigits: 1 })} pal</p>
          </div>
        ))}
        {allItens.length > 5 && <p className="text-slate-600 text-xs text-center py-2">+ {allItens.length - 5} produtos</p>}
      </div>

      <div className="flex gap-3">
        <button onClick={onVoltar} className="flex-1 bg-[#112240] border border-[#1E3A5F] text-slate-400 font-semibold py-4 rounded-2xl text-sm flex items-center justify-center gap-2">
          <ChevronLeft size={16} />Voltar
        </button>
        <button onClick={onConfirmar} disabled={iniciando}
          className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-semibold py-4 rounded-2xl text-sm flex items-center justify-center gap-2 transition-colors">
          {iniciando
...
function SRow({ label, value, highlight }) {
  return (
    <div className="flex justify-between items-center gap-4 px-4 py-3">
      ...
      <span className={`text-xs font-medium text-right ${highlight ? 'text-orange-400' : 'text-slate-300'}`}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex gap-3 pt-2">
      ...
      <button onClick={onProximo} disabled={!podeProximo}
        className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 disabled:pointer-events-none text-white font-semibold py-4 rounded-2xl text-sm flex items-center justify-center gap-2 transition-colors">
...
function ViagemAtiva({ viagem, pedidos, tarefaStatus, onVerificarTarefa, showNF, setShowNF, numeroNF, setNumeroNF, registrando, setRegistrando, registrarEtapa }) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-5 space-y-4">
      {/* Trip header */}
      <div className="bg-[#112240] rounded-2xl border border-orange-500/30 p-4 space-y-3">
        <div className="flex items-center gap-2">
          <MapPin size={14} className="text-orange-400 shrink-0" />
          <p className="text-orange-400 font-semibold text-sm">{viagem?.unidade?.nome}</p>
          <span className="text-slate-600 text-xs">— {viagem?.unidade?.cidade}</span>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-[#0B1929] rounded-xl px-3 py-2">
            <p className="text-slate-600 text-[10px] uppercase tracking-widest">Carreta</p>
            <p className="text-white font-mono font-semibold text-sm mt-0.5">{viagem?.carreta?.placa}</p>
            <p className="text-slate-600 text-[10px]">{viagem?.carreta?.tipo}</p>
          </div>
          <div className="bg-[#0B1929] rounded-xl px-3 py-2">
            <p className="text-slate-600 text-[10px] uppercase tracking-widest">Cavalo</p>
            <p className="text-white font-mono font-semibold text-sm mt-0.5">{viagem?.cavalo?.placa}</p>
            <p className="text-slate-600 text-[10px]">{viagem?.cavalo?.tipo}</p>
          </div>
        </div>
        <div className="bg-[#0B1929] rounded-xl px-3 py-2">
          <p className="text-slate-600 text-[10px] uppercase tracking-widest mb-1">Pedidos</p>
          <div className="flex flex-wrap gap-2">
            {numerosUnicos.map(n => <span key={n} className="text-orange-400 font-mono text-xs font-semibold">#{n}</span>)}
          </div>
      ...
      <div className="space-y-2">
        {ETAPAS.map((etapa, i) => {
          ...
          if (done) return (
            <div key={etapa.key} className="bg-[#112240] rounded-2xl border border-[#1E3A5F] px-4 py-3 flex items-center gap-3">
              <CheckCircle size={18} className="text-green-400 shrink-0" />
              <div>
                <p className="text-slate-300 text-sm font-medium">{etapa.label}</p>
                <p className="text-slate-600 text-xs mt-0.5">{formatTs(viagem[etapa.field])}</p>
              </div>
            </div>
          )

          if (current && bloqueada) return (
            <div key={etapa.key} className="bg-[#112240] rounded-2xl border border-[#1E3A5F] p-4">
              <div className="flex items-center gap-2 mb-3">
                <Lock size={15} className="text-slate-600 shrink-0" />
                <p className="text-slate-500 font-semibold text-sm">{etapa.label}</p>
              </div>
              <div className="bg-[#0B1929] rounded-xl px-4 py-3 flex items-center justify-between gap-3">
                <p className="text-slate-600 text-xs">Aguardando conclusão da conferência na revenda</p>
                <button onClick={onVerificarTarefa}
                  className="text-orange-400 text-xs font-semibold shrink-0 flex items-center gap-1 hover:text-orange-300 transition-colors">
          ...
          if (current) return (
            <div key={etapa.key} className="bg-orange-500/10 rounded-2xl border-2 border-orange-500 p-4">
              <div className="flex items-center gap-2 mb-4">
                <etapa.Icon size={16} className="text-orange-400" />
                <p className="text-orange-400 font-semibold text-sm uppercase tracking-wide">{etapa.label}</p>
              </div>
              <button onClick={() => handleEtapa(etapa)} disabled={registrando}
                className="w-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-bold py-5 rounded-xl text-base transition-colors flex items-center justify-center gap-3">
                {registrando
          ...
          return (
            <div key={etapa.key} className="bg-[#112240] rounded-2xl border border-[#1E3A5F] px-4 py-3 flex items-center gap-3 opacity-40">
      ...
      {pedidos.length > 0 && (
        <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1E3A5F] flex items-center gap-2">
            ...
            <p className="text-slate-600 text-[10px] font-semibold uppercase tracking-widest">{pedidos.length} produtos da carga</p>
          </div>
          {pedidos.slice(0, 5).map((item, i) => (
            <div key={item.id} className={`flex items-center justify-between px-4 py-2.5 ${i < Math.min(pedidos.length, 5) - 1 ? 'border-b border-[#0B1929]' : ''}`}>
          ...
          {pedidos.length > 5 && <p className="text-slate-600 text-xs text-center py-2">+ {pedidos.length - 5} produtos</p>}
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end">
      <div className="w-full max-w-lg mx-auto bg-[#112240] rounded-t-3xl p-6 space-y-5">
        ...
        <div>
          <p className="text-white font-semibold text-base">Chegada na Revenda</p>
          <p className="text-slate-500 text-sm mt-1">Informe o número da Nota Fiscal a entregar</p>
        </div>
        <div>
          <label className="block text-slate-500 text-[11px] font-semibold uppercase tracking-widest mb-1.5">
            Número da NF <span className="text-orange-500">*</span>
          </label>
          <input ref={ref} value={numeroNF} onChange={e => setNumeroNF(e.target.value)}
            ...
            className="w-full bg-[#0B1929] border border-[#1E3A5F] rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-orange-500" />
        </div>
        <div className="flex gap-3">
          <button onClick={onCancelar} className="flex-1 bg-[#0B1929] border border-[#1E3A5F] text-slate-400 font-semibold py-4 rounded-2xl text-sm">Cancelar</button>
          <button onClick={onConfirmar} disabled={!numeroNF.trim()}
            className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-semibold py-4 rounded-2xl text-sm transition-colors">
            Confirmar Chegada
          </button>
        </div>
      </div>
...
function MetricBox({ label, value, highlight }) {
  return (
    <div className={`rounded-2xl border p-3 text-center ${highlight ? 'bg-orange-500/10 border-orange-500/30' : 'bg-[#112240] border-[#1E3A5F]'}`}>
      <p className={`text-[10px] mb-1 uppercase tracking-wide ${highlight ? 'text-orange-400' : 'text-slate-600'}`}>{label}</p>
      <p className={`font-bold text-base font-mono ${highlight ? 'text-orange-400' : 'text-white'}`}>{value}</p>
    </div>
  )
}
      ...
      <main className="flex-1 overflow-y-auto pb-28">
        <div className="max-w-lg mx-auto px-4 pt-5 space-y-4">
          ...
          <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-5 text-center">
            ...
            <p className="text-white font-bold text-base">Viagem Concluída!</p>
          ...
          {/* Horários */}
          <section>
            <p className="text-[11px] text-slate-600 font-semibold uppercase tracking-widest mb-2 px-1">Horários</p>
            <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] divide-y divide-[#1E3A5F]">
          ...
          {/* TMA */}
          <section>
            <p className="text-[11px] text-slate-600 font-semibold uppercase tracking-widest mb-2 px-1">TMA — Tempo de Atendimento</p>
          ...
          {/* TMV */}
          <section>
            <p className="text-[11px] text-slate-600 font-semibold uppercase tracking-widest mb-2 px-1">TMV — Tempo de Viagem</p>
          ...
          {/* Paletes */}
          <div className="bg-[#112240] rounded-2xl border border-[#1E3A5F] p-4">
            <p className="text-[11px] text-slate-600 font-semibold uppercase tracking-widest mb-3">Paletes</p>
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                <p className="text-slate-600 text-[10px] mb-1">Esperado</p>
                <p className="text-white font-bold text-xl">{paletesEsp.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}</p>
              </div>
              <div>
                <p className="text-slate-600 text-[10px] mb-1">Recebido</p>
                <p className="text-white font-bold text-xl">{paletesRec.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}</p>
              </div>
              <div>
                <p className="text-slate-600 text-[10px] mb-1">Diferença</p>
                <p className={`font-bold text-xl ${diferenca === 0 ? 'text-green-400' : 'text-orange-400'}`}>
          ...
          {stats.total_anomalias > 0 ? (
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-orange-400 text-sm">{stats.total_anomalias} anomalia{stats.total_anomalias > 1 ? 's' : ''} registrada{stats.total_anomalias > 1 ? 's' : ''}</p>
            </div>
          ) : (
