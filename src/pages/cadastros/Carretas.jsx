...
export default function Carretas() {
  ...
  const salvar = async (e) => {
    ...
    if (editando) {
      ...
      if (error) setErro(error.message === 'duplicate key value violates unique constraint "carretas_placa_key"'
        ? 'Já existe uma carreta com esta placa.' : error.message)
      else { await carregar(); fechar() }
    } else {
      const { error } = await supabase.from('carretas').insert({ placa: placaFormatada, tipo })
      if (error) setErro(error.message.includes('duplicate key')
        ? 'Já existe uma carreta com esta placa.' : error.message)
      else { await carregar(); fechar() }
    }
    setSalvando(false)
  ...
  return (
    <div className="px-5 py-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-white font-semibold text-sm">{lista.length} carreta{lista.length !== 1 ? 's' : ''}</p>
          <p className="text-slate-600 text-xs">{lista.filter(c => c.ativo).length} ativa{lista.filter(c => c.ativo).length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {loading ? (
        ...
        <p className="text-center text-slate-600 text-sm py-12">Nenhuma carreta encontrada</p>
      ) : (
            ...
            <div key={c.id} className="bg-[#0F1E33] rounded-xl p-4 border border-[#1E3A5F]">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-[#0B1929] border border-[#1E3A5F] flex items-center justify-center shrink-0">
                    <Truck size={16} className="text-slate-500" />
                  </div>
                  <div>
                    <p className="text-white font-bold text-base tracking-wider font-mono">{c.placa}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[11px] bg-[#0B1929] border border-[#1E3A5F] text-slate-400 px-2 py-0.5 rounded-full font-medium">{c.tipo}</span>
        ...
        <Modal title={editando ? 'Editar Carreta' : 'Nova Carreta'} onClose={fechar}>
          <form onSubmit={salvar} className="space-y-4">
            <Field label="Placa" required>
              ...
              <p className="text-slate-600 text-xs mt-1">Formato antigo: ABC1234 · Mercosul: ABC1D23</p>
            </Field>
