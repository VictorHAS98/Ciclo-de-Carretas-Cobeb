  ...
  return (
    <div className="px-5 py-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-cobeb-text font-semibold text-sm">{lista.length} carreta{lista.length !== 1 ? 's' : ''}</p>
          <p className="text-slate-500 text-xs">{lista.filter(c => c.ativo).length} ativa{lista.filter(c => c.ativo).length !== 1 ? 's' : ''}</p>
        </div>
