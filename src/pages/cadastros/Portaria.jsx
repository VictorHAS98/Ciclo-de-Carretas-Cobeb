  ...
  return (
    <div className="px-5 py-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-cobeb-text font-semibold text-sm">{lista.length} porteiro{lista.length !== 1 ? 's' : ''}</p>
      ...
      {loading ? (
        ...
        <p className="text-center text-slate-500 text-sm py-12">Nenhum porteiro encontrado</p>
      ) : (
      ...
      {modal && (
        <Modal title={editando ? 'Editar Porteiro' : 'Novo Porteiro'} onClose={fechar}>
          {senhaCriada && !editando ? (
            ...
            <form onSubmit={salvar} className="space-y-4">
              ...
              <button type="submit" disabled={salvando}
                ...
                {salvando ? 'Salvando...' : editando ? 'Salvar alterações' : 'Criar porteiro'}
              </button>
            </form>
          )}
        </Modal>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
        ...
        <p className="text-slate-500 text-xs mb-1">Login do porteiro:</p>
